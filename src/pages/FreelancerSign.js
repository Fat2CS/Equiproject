...
const descriptionRef = useRef();
	...
	const handleSignIn = async (e) => {
		...
		try {
			...
			await setDoc(doc(db, "FreelancerRequest", userId), {
				userId: userId,
				email: email,
				type: type,
        name:name,
        ...
        level: level,
			...
			router.push(`./ValidateFreelancer`);
		} catch (error) {
					...
					<div>
						...
						<div className="mt-2">
							<textarea
								...
								required
								ref={descriptionRef}
								className="block w-full rounded-lg border py-3.5 px-4 text-letter-grey focus:ring-letter-orange-600 focus:bg-black sm:text-sm sm:leading-6 placeholder-letter-black-button resize-none"
								rows={4}
							/>
						</div>
					</div>
